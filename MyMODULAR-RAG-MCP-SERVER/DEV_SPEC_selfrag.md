<!-- Self-RAG evolution specification based on the original DEV_SPEC.md. -->
# Developer Specification — Self-RAG Edition (DEV_SPEC_selfrag)

> 版本：0.2 — Self-RAG 架构与训练闭环改造版  
> 基线：`DEV_SPEC.md`（保留原 RAG/MCP/可观测性主体设计）  
> 更新时间：2026-09-01
...
- **按需检索 (Adaptive Retrieval)**：模型先产生 `RetrieveDecision`，将问题分为无需检索、必须检索或低置信度待检索三类；只有命中策略阈值时才调用原 Hybrid Search。
- **证据反思 (Evidence Reflection)**：对每个候选 Chunk 预测 Relevant/Irrelevant，生成回答段落后再判断 Fully/Partially/Not Supported，避免“检索到了但没有被答案正确使用”。
- **效用评估 (Utility Critique)**：为候选答案或段落预测 1–5 级效用分数，结合相关性、支持度和引用完整度进行可配置打分。
- **受控迭代 (Bounded Retry)**：当证据不足或输出效用低于阈值时，可以重写查询并再次检索；通过 `max_retrieval_rounds` 和预算限制防止无限循环。
- **Reflection Token 微调**：将检索需求、证据相关性、事实支持度和回答效用统一编码为扩展词表中的特殊 Token，使 Student 模型在推理时能够学习检索、生成与自我批判。

### Self-RAG 训练与蒸馏闭环

- **样本构建**：从项目文档、Golden Set、真实/合成 Query 和检索 Trace 构建正例、硬负例、无需检索样本与多跳样本。
- **Teacher 标注**：默认调用 `kimi-k3` 生成结构化 Reflection Labels 与参考答案，调用失败、质量不足或预算受限时切换 `deepseek-v4-pro`；支持双教师一致性校验。
- **质量控制**：使用 JSON Schema、规则校验、标签一致性、证据蕴含检查、去重、污染检测和按文档分组切分，防止错误标签与数据泄漏进入训练集。
- **两阶段训练**：先训练 Critic 学习 Reflection Labels，再将离线 Critic 标签插入 Generator 数据，训练能够产生回答与 Reflection Tokens 的 Student。
- **参数高效微调**：默认采用 LoRA/QLoRA 完成可复现训练；保留全量微调、DPO/偏好优化和多模型对比扩展点。
- **闭环迭代**：线上低置信度 Trace 与人工纠错进入候选池，经重新标注和离线评估后形成新数据版本，不直接在线自学习。
...
- **Self-RAG 策略插拔 (Reflection Strategy)**：
    - `disabled`：保持传统 Hybrid RAG 行为，作为兼容模式和实验基线；
    - `prompted`：使用通用强模型通过结构化 Prompt 完成检索门控与反思，便于训练前验证；
    - `finetuned`：使用微调后的 Student 直接预测 Reflection Tokens，作为目标生产模式；
    - `hybrid`：Student 低置信度时调用 Teacher 或规则校验器兜底。
...
- **可视化管理平台 (Visual Management Dashboard)**：
    - 基于 Streamlit 的本地 Web 管理面板，先按原计划交付六大基础页面，再在阶段 M 增加 Self-RAG Trace 与 Training Monitor 两个页面：
        ...
        - **Self-RAG Trace（阶段 M）**：查看检索门控、证据过滤、段落支持度、效用、重试和回退。
        - **Training Monitor（阶段 M）**：查看 Dataset、Teacher 成本、训练 Run、评测和模型注册状态。
...
### 3.6 Self-RAG 在线推理设计 (Self-Reflective Inference)

> 设计依据：Self-RAG 原论文提出通过扩展词表中的 Reflection Tokens，让同一模型学习按需检索、评估证据相关性、判断生成内容是否被证据支持，并通过分段解码选择更高效用的输出。本文在其思想上做工程化适配，复用本项目已有 Hybrid Search/Rerank/MCP 基础设施。  
> 参考：[Self-RAG Paper](https://arxiv.org/abs/2310.11511) ｜ [Official Implementation](https://github.com/AkariAsai/self-rag)

#### 3.6.1 Reflection Token 与领域对象

Reflection Token 的**语义名称稳定**，实际 Token 字符串由 tokenizer manifest 配置。默认映射如下：

| 维度 | 语义枚举 | 默认 Token | 含义 |
|------|---------|-----------|------|
| Retrieval | `RETRIEVE` | `<RET_YES>` | 当前问题/段落需要外部知识 |
| Retrieval | `NO_RETRIEVE` | `<RET_NO>` | 可直接回答，不触发检索 |
| Relevance | `RELEVANT` | `<REL_YES>` | 当前证据与问题/生成段相关 |
| Relevance | `IRRELEVANT` | `<REL_NO>` | 当前证据无关，应从上下文剔除 |
| Support | `FULLY_SUPPORTED` | `<SUP_FULL>` | 生成段中的可验证陈述均被证据支持 |
| Support | `PARTIALLY_SUPPORTED` | `<SUP_PARTIAL>` | 仅部分陈述被证据支持 |
| Support | `NOT_SUPPORTED` | `<SUP_NONE>` | 证据不能支持生成段 |
| Utility | `UTILITY_1..5` | `<UTIL_1>` ... `<UTIL_5>` | 从错误/无帮助到完整/高质量的五级效用 |

Reflection Token 只表达**对证据与生成结果的判断**，不表达**控制动作**。是否重写 Query、是否继续下一轮检索、是否最终拒答，全部由外部 `SelfRAGPolicy` 依据上述判断标签、阈值与预算决定，模型不emit `RETRY`/`ABSTAIN` 之类的控制 Token。这样 Critic 的训练目标保持纯评判性，控制流可以在不重新训练模型的前提下调整。

核心类型统一放入 `src/core/self_rag_types.py`，不污染原 `Document/Chunk/RetrievalResult` 契约：
...
所有概率进入类型前必须为有限数并归一化到 `[0, 1]`。解析失败时不得伪造置信度。
...
QueryProcessor（原有）
    │
    ▼
Retrieval Gate ── <RET_NO> ───────────────► Direct Generation
    │ <RET_YES>                                  │
    ▼                                            │
Hybrid Search + Rerank（原有）                   │
    │                                            │
    ▼                                            │
Evidence Critic: Relevant / Irrelevant           │
    │                                             │
    ├── 无相关证据 ─► Query Rewrite ─► Retry ────┤
    │                  （受 max_rounds 限制）      │
    ▼                                             │
Segment Generator ◄───────────────────────────────┘
    │
    ▼
Support Critic + Utility Critic
    │
    ├── Supported + Utility 达标 ─► 接受段落
    ├── 证据不足且可重试 ──────────► Rewrite/Retrieve
    └── 超预算或仍无支持 ──────────► Abstain/降级
    │
    ▼
ResponseBuilder（清理控制 Token、组装引用与可选诊断信息）
```

终止条件必须同时受以下配置限制：

- `max_retrieval_rounds`：默认 2，最大不超过 5；
- `max_generation_segments`：限制长答案的分段数量；
- `max_total_context_tokens`：防止多轮检索无限膨胀；
- `max_query_cost`：Teacher/API 模式下的单请求预算；
- `wall_time_ms`：超过端到端时限时触发降级；
- Policy 依据支持度/效用阈值判定“可接受”或“证据不足需拒答”时立即终止。

#### 3.6.3 SelfRAGOrchestrator 职责

新增 `src/core/self_rag/`，由以下独立组件组成：

| 组件 | 职责 | 不负责 |
|------|------|-------|
| `RetrievalGate` | 判断是否检索，输出决策、概率和策略来源 | 不直接执行检索 |
| `EvidenceCritic` | 对候选 Chunk 做相关性判定与过滤 | 不改变向量库分数 |
| `SegmentGenerator` | 基于 Query、已接受段落和证据生成下一段 | 不解析原始 Provider 响应 |
| `SupportCritic` | 判断生成段是否受所引用证据支持 | 不生成最终引用格式 |
| `UtilityCritic` | 输出 1–5 级效用分布 | 不单独决定重试 |
| `QueryRewriter` | 基于失败类型重写检索 Query | 不修改用户原始问题 |
| `SelfRAGPolicy` | 根据概率、阈值、预算决定接受/重试/拒答 | 不调用外部模型 |
| `SelfRAGOrchestrator` | 编排状态机、收集 Trace、调用原 Retrieval/Rerank | 不包含 Provider 分支判断 |

三种运行实现共用同一抽象接口：

```python
class BaseReflectionModel(ABC):
    def decide_retrieval(self, request: ReflectionRequest) -> ReflectionPrediction: ...
    def assess_evidence(self, request: EvidenceRequest) -> list[EvidenceAssessment]: ...
    def generate_segment(self, request: SegmentRequest) -> RawModelResponse: ...
    def assess_support(self, request: SupportRequest) -> ReflectionPrediction: ...
    def assess_utility(self, request: UtilityRequest) -> ReflectionPrediction: ...
```

- `PromptedReflectionModel`：调用通用 API LLM，使用 JSON Schema，训练前即可联调。
- `FineTunedReflectionModel`：调用本地/远程 Student，读取控制 Token 概率。
- `HybridReflectionModel`：Student 高置信度时直接使用，低置信度时交给 Teacher/规则兜底。

#### 3.6.4 LLM Response 解析与规范化

原有 `BaseLLM.generate() -> str` 保持兼容，同时新增结构化原始响应：
...
class RawModelResponse:
    ...
    reasoning_text: str | None = None  # 只驻留内存，不写入训练集/Trace/MCP
    provider_metadata: dict[str, Any] = field(default_factory=dict)
```

新增 `SelfRAGResponseParser`，严格按以下优先级解析：

1. **Provider 原生结构化输出**：验证 JSON Schema，拒绝缺字段、未知枚举和越界概率。
2. **控制 Token 状态机**：扫描 tokenizer 注册的完整 Token ID，不使用脆弱的普通字符串 `split()`。
3. **受限 JSON 提取**：只允许从单个 fenced JSON block 或完整 JSON 对象恢复，不使用贪婪正则拼接多个对象。
4. **修复重试**：最多一次低温格式修复请求；修复 Prompt 只包含错误信息和原响应，不扩展业务事实。
5. **确定性降级**：仍失败时记录 `parse_error`，根据配置回退传统 RAG 或返回可解释错误，绝不把未解析文本当作已验证结果。

解析器必须处理：

- Streaming 中控制 Token 被拆分在多个 chunk；
- Token 重复、顺序错误、缺少结束 Token；
- JSON 与自然语言混合、Markdown code fence、Unicode 转义；
- Provider 将 reasoning 与 final answer 分字段返回；
- logprobs 缺失时使用显式 `confidence=null`，不得自行构造概率；
- 最终面向用户的 `answer` 必须移除所有内部控制 Token；
- MCP 返回不得包含隐藏 reasoning、教师原始判定文本、API Key 或 Provider 敏感 metadata。

#### 3.6.5 分段解码与候选打分

MVP 使用单候选贪心/采样生成；增强模式支持每段 `beam_size` 个候选。候选分数由配置化权重计算：

```text
segment_score =
    w_rel     * log P(RELEVANT)
  + w_support * log P(FULLY_SUPPORTED)
  + w_utility * E[UTILITY / 5]
  + w_lm      * normalized_lm_logprob
  - w_cost    * retrieval_and_token_cost
  - w_retry   * retry_count
```

- 所有 `log(0)` 使用 epsilon 截断；
- 权重必须记录在 Trace 和评估报告中；
- `beam_size=1` 时不引入额外候选开销；
- 硬约束可直接过滤 `IRRELEVANT` 或 `NOT_SUPPORTED` 候选；
- 软约束用于实验调权，不允许在测试中只凭主观样例选择权重。

#### 3.6.6 对外接口与兼容模式

`query_knowledge_hub` 保持原参数，新增可选字段：
...
基础返回继续包含 `answer`、`citations`、`images`；开启诊断时增加：
...
```json
{
  "self_rag": {
    ...
    "segment_assessments": [
      {"support": "fully_supported", "utility": 5, "evidence_ids": ["chunk_001"]}
    ]
  }
}
```

诊断对象只包含可审计结论和 ID，不返回隐藏推理。`self_rag.enabled=false` 或 Self-RAG 组件初始化失败时，使用原 Hybrid Search + ResponseBuilder，且在 Trace 中明确记录 `fallback_reason`。

---

### 3.7 训练样本构建与 Teacher 蒸馏 Pipeline

#### 3.7.1 数据来源与样本类型

数据源按优先级分为：

1. **项目内真实知识**：经 Ingestion 后的文档、Chunk、标题、图片 Caption 与引用关系。
2. **人工/Golden QA**：`golden_test_set.json`、人工验收问题、错误案例与修正答案。
3. **真实查询 Trace**：只使用已脱敏、获得授权且通过质量审核的 Query；线上日志默认不自动进入训练集。
4. **合成 Query**：Teacher 基于文档生成事实问答、跨 Chunk、多跳、摘要、比较、不可回答和无需检索问题。
5. **检索候选**：正证据、BM25/Dense/RRF 召回结果、Cross-Encoder 难负例、同主题但不支持答案的对抗负例。

必须覆盖的训练任务：

| 任务 | 输入 | 标签/输出 |
|------|------|----------|
| Retrieval Decision | instruction + optional history | retrieve / no_retrieve |
| Evidence Relevance | instruction + passage | relevant / irrelevant |
| Support Judgment | instruction + passage + answer segment | full / partial / none |
| Utility Judgment | instruction + complete/partial answer | 1..5 |
| Query Rewrite | instruction + failed evidence summary | rewritten query |
| Grounded Generation | instruction + relevant passages | answer segments + citations + reflection tokens |
| Insufficient Evidence | instruction + insufficient/contradictory evidence | not_supported + 低 utility + concise reason（是否拒答由 Policy 决定，不作为标签） |

数据比例由配置控制，初始建议只是默认实验起点：知识密集型检索样本 50%、无需检索样本 15%、无关/难负例 15%、部分支持与冲突证据 10%、不可回答/拒答 10%。任何比例调整必须保存在 Dataset Manifest。

#### 3.7.2 数据流水线

```
Source Snapshot / Golden QA / Approved Traces
                    │
                    ▼
Seed Collector ─► PII/License Filter ─► Canonical Samples
                    │
                    ▼
Synthetic Query Generator (Teacher)
                    │
                    ▼
Candidate Retriever (Dense + Sparse + RRF + Rerank)
                    │
                    ├── Positive Evidence
                    ├── Hard Negatives
                    ...
                    │
                    ▼
Teacher Labeler (Kimi K3 primary / DeepSeek V4 fallback)
                    │
                    ▼
Schema Validation ─► Rule Checks ─► Agreement/Adjudication
                    │
                    ▼
Dedup + Contamination Check + Source-group Split
                    │
                    ▼
Critic JSONL / Generator JSONL / Preference JSONL
                    │
                    ▼
Dataset Manifest + Quality Report + Cost Report
```

每个阶段必须支持：输入哈希、幂等输出、分片、并发限制、指数退避、断点续跑、失败队列和 dry-run。不得因为单条 Teacher API 失败而重跑整个数据集。

#### 3.7.3 统一样本 Schema

标准存储采用 JSONL，统计/分析可额外导出 Parquet。单条记录示例：
...
```json
{
  ...
  "labels": {
    "retrieve": "retrieve",
    "evidence": [{"chunk_id": "chunk_001", "relevance": "relevant"}],
    "segments": [
      {
        "text": "...",
  ...
  "teacher": {
    "provider": "kimi",
    "model": "kimi-k3",
    "prompt_version": "support-label-v1",
    "request_id": "...",
    "temperature": 0,
  ...
  "quality": {
    "schema_valid": true,
    "rule_checks": ["citation_exists", "answer_entails_evidence"],
    "agreement": "single_teacher_high_confidence",
...
原始 Teacher Response 如需留存，仅写入访问受控的 quarantine 存储并设置保留期；训练数据仅保存规范化结果和原始响应哈希。

#### 3.7.4 Teacher Provider 与模型策略

Teacher 使用独立 `BaseTeacherLLM`，不复用在线 Student 配置，避免密钥、预算和采样参数相互污染：

| 优先级 | Provider | 默认模型标识 | 用途 |
|-------|----------|-------------|------|
| Primary | Kimi API | `kimi-k3` | 长上下文样本生成、Reflection 标签、答案与证据审查 |
| Fallback | DeepSeek API | `deepseek-v4-pro` | Kimi 不可用、超预算或低置信度时的替代/复核 |
| Cost-sensitive | DeepSeek API | `deepseek-v4-flash` | 大规模初筛、简单 relevance/utility 标签 |
| Test | FakeTeacherLLM | `fake-teacher` | 单元测试、离线开发、确定性回放 |

Kimi 官方 API 将 `kimi-k3` 定义为旗舰模型并提供 OpenAI 兼容接口；DeepSeek 官方 API 当前公开 `deepseek-v4-pro` 与 `deepseek-v4-flash`。具体 alias 可能随服务升级，因此启动标注任务前必须调用模型列表/最小探针验证，不依据文档日期猜测可用性。  
参考：[Kimi API Overview](https://www.kimi.ai/help/kimi-api/api-overview) ｜ [DeepSeek API Docs](https://api-docs.deepseek.com/) ｜ [DeepSeek V4 Model Card](https://fe-static.deepseek.com/chat/transparency/deepseek-V4-model-card-EN.pdf)
...
禁止在配置仓库中提交真实密钥。Teacher 输出只有通过 Schema 和证据检查后才能进入 accepted 数据集。

#### 3.7.5 标注 Prompt 与一致性控制

每个标签任务使用独立版本化 Prompt，放在 `config/prompts/self_rag/teacher/`：

- Prompt 明确给出允许的枚举、字段解释、正反例和“证据不足时不得补充常识”的约束；
- 结构化输出只包含标签、置信度、短理由、证据 ID 和参考答案；
- Relevance、Support、Utility 分开标注，避免一个错误判断污染全部字段；
- 高风险样本采用 Kimi + DeepSeek 双教师；一致则接受，不一致进入 adjudication；
- adjudication 默认由主 Teacher 在不看到模型身份的情况下复核，关键评测集可人工审核；
- Prompt、Provider、模型 alias、请求参数与响应哈希写入 manifest；
- 禁止把教师隐藏 reasoning 当作 Student 训练目标。

#### 3.7.6 质量过滤、去重与数据切分

质量门禁按顺序执行：

1. JSON Schema 与枚举合法性；
2. `evidence_ids` 必须存在于当前 retrieval snapshot；
3. `<RET_NO>` 样本不得同时依赖外部证据才能成立；
4. `FULLY_SUPPORTED` 样本中的原子事实必须能被引用 Chunk 覆盖；
5. `NOT_SUPPORTED`/不可回答样本不得在答案中泄漏 Teacher 常识补全；
6. MinHash/Embedding 双重去重，防止模板化重复样本过度占比；
7. 检测 benchmark、Golden QA 和评估答案污染；
8. 按 `source_group_id` 分组切分 train/dev/test，禁止同文档 Chunk 跨集合泄漏；
9. 统计标签分布、长度分布、语言分布、Provider 分布与拒绝率；
10. 质量低于阈值的数据进入 quarantine，而不是静默删除。

默认切分为 80/10/10，但必须以文档/主题组为单位，固定 seed，并将实际组 ID 哈希写入 manifest。

#### 3.7.7 数据版本与产物

```
data/training/
├── raw/                    # 只读来源快照/授权 Trace（默认 gitignore）
├── staging/                # 候选与未决标签
├── quarantine/             # 失败/冲突/敏感样本
├── processed/
│   └── selfrag-v0.1.0/
│       ├── critic_train.jsonl
│       ├── critic_dev.jsonl
...
│       ├── quality_report.json
│       └── cost_report.json
└── cache/                  # Teacher 响应哈希缓存
```

Manifest 至少记录：父版本、Git commit、输入快照哈希、索引版本、Prompt 版本、Teacher 模型、token/cost 统计、过滤计数、标签分布、切分 seed 与生成命令。

---

### 3.8 Critic / Generator 模型训练 Pipeline

#### 3.8.1 训练策略

采用与 Self-RAG 原始方法一致的两阶段主线，同时适配个人/小团队算力：

1. **Critic SFT**：训练 Student/Critic 根据 instruction、passage、answer segment 预测 Reflection Tokens。
2. **离线打标**：使用训练后的 Critic 为 Generator 原始指令数据补充 Reflection Tokens；低置信度样本仍交给 Teacher。
3. **Generator SFT**：训练 Student 在文本生成过程中预测检索与批判 Token，并生成带引用的答案段落。
4. **可选偏好优化**：在稳定 SFT 基线后，使用 DPO/ORPO 训练更偏好高支持度、高效用、低无效检索的候选。
5. **部署校准**：在 dev set 上选择检索阈值、支持度阈值、效用阈值和候选打分权重，不在 test set 调参。

MVP 可以使用同一个 base model 分别训练 Critic Adapter 与 Generator Adapter；生产目标是 Generator 单模型在推理时完成 Reflection Token 预测，避免长期依赖 Teacher。

#### 3.8.2 技术栈与可替换接口

- PyTorch：训练运行时；
- Hugging Face Transformers：Tokenizer、CausalLM、Trainer 基础能力；
- Datasets：JSONL/Parquet 读取、map、shard 与 streaming；
- PEFT：LoRA/QLoRA Adapter；
- TRL：可选 SFT/DPO 训练器；
- Accelerate / DeepSpeed / FSDP：多卡与显存优化，可配置选择；
- bitsandbytes：可选 4-bit/8-bit 量化训练；
- safetensors：权重落盘；
- MLflow 或本地 JSONL Tracker：实验参数与指标记录，默认本地可运行。

所有框架代码封装在 `src/training/backends/`，主 Pipeline 不直接依赖单一 Trainer 类。

#### 3.8.3 Tokenizer 与 Loss Mask

- 在训练前一次性注册全部 Reflection Tokens，并保存 `tokenizer_manifest.json`；
- 检查每个控制 Token 映射为单一、唯一且非 UNK 的 token ID；
- 调整模型 embedding 大小后验证输入/输出 embedding 是否正确绑定；
- Passage Context 默认只作为条件输入，其 token label 设为 `-100`，不要求模型背诵检索文档；
- 对用户指令、系统模板和 padding 同样 mask，只在答案与 Reflection Tokens 上计算 Causal LM Loss；
- 可为 Reflection Tokens 设置单独权重，防止它们在长文本 loss 中被淹没；
- 训练与服务必须加载同一个 tokenizer manifest，启动时进行哈希校验。
...
<PASSAGE id="chunk_001">{passage}</PASSAGE>
<ANSWER_SEGMENT>{segment}</ANSWER_SEGMENT>
<LABEL>
```

目标按任务分别为 `<RET_*>`、`<REL_*>`、`<SUP_*>` 或 `<UTIL_*>`。Critic 训练要求：

- 多任务 sampling 权重可配置；
- 训练/验证每个任务单独计算 macro-F1、per-class precision/recall 和 calibration error；
- 保存 best-by-macro-F1 与 last checkpoint；
- 类别失衡时优先采用采样/权重，不通过复制测试集样本解决；
- 低置信度与教师不一致样本不得进入 dev/test；
- 支持 LoRA/QLoRA、梯度累积、混合精度、gradient checkpointing 和断点续训。

#### 3.8.5 Generator 训练

Generator 样本将 Reflection Tokens 与答案片段交错：

```text
<RET_YES>
<EVIDENCE id="chunk_001">...</EVIDENCE>
<REL_YES>
答案第一段 [1]
<SUP_FULL><UTIL_5>
答案第二段 [2]
...
- 同时包含 `<RET_NO>` 直接回答样本，避免模型形成“逢问必搜”；
- 加入 irrelevant passage 和 partially-supported 对抗样本；
- 生成文本中的 `[n]` 必须能映射到当前证据列表；
- context、prompt 和非目标元数据 mask 掉 loss；
- 报告普通文本 token loss 与 reflection token loss；
- dev 评估运行真实状态机，而不只看 perplexity；
- 最少保留传统 RAG 基线和 Prompted Self-RAG 基线用于对比。

#### 3.8.6 可选偏好训练

只有 SFT 通过发布门禁后才启用偏好阶段。候选对可以来自不同 beam、不同检索轮次或 Teacher 修订：

- chosen：事实受支持、引用正确、效用高、无多余检索；
- rejected：引用无关、事实不受支持、过度检索、答非所问或未合理拒答；
- 偏好标签必须有自动指标或人工/Teacher 判定依据；
- 不以隐藏 reasoning 长短作为偏好；
- DPO 训练单独版本化，必须与 SFT checkpoint 做消融比较。

#### 3.8.7 TrainingRun、断点续训与产物

```yaml
training:
  base_model: ${SELF_RAG_BASE_MODEL}
  output_dir: ./artifacts/models/selfrag
  seed: 42
  precision: bf16
  method: qlora                 # lora | qlora | full
  stages: [critic_sft, generator_sft]
  max_seq_length: 8192
  ...
  lora:
    ...
    target_modules: auto
  tracking:
    backend: jsonl              # jsonl | mlflow
    log_dir: ./artifacts/runs
```

每个训练运行必须输出：

- `run_manifest.json`：代码 commit、配置、Dataset/Tokenizer 哈希、设备、依赖版本；
- `checkpoints/`：可恢复 optimizer/scheduler/RNG 状态；
- `adapter/` 或 `model/`：safetensors 权重；
- `tokenizer/` 与 `tokenizer_manifest.json`；
- `train_metrics.jsonl`、`eval_metrics.json`、混淆矩阵；
- `model_card.md`：用途、数据、限制、许可证、评测和风险；
- `reproduce.ps1` / `reproduce.sh`：不含密钥的复现实验命令。

断点续训必须校验 Dataset、Tokenizer 和关键超参哈希。出现不兼容时拒绝静默续训并提示开启新 Run。

#### 3.8.8 硬件档位与最小训练闭环

规格不将单一 GPU 型号写死，提供三个可验收档位：

| 档位 | 目标 | 默认策略 |
|------|------|---------|
| Smoke | 验证数据、loss mask、checkpoint 与推理可用 | Tiny model / 32–128 样本 / CPU 或单卡 |
| Local | 个人工作站训练可用 Student | 1B–7B + QLoRA + 单卡/双卡 |
| Scale | 完整数据与更大 Student | Accelerate/DeepSpeed/FSDP 多卡 |

任何生产训练前必须先通过 Smoke：10–50 step loss 有限且下降、checkpoint 可恢复、控制 Token 可生成、离线推理可被 Parser 正确解析。

#### 3.8.9 模型发布门禁

候选模型只有同时满足以下条件才能标记为 deployable：

- Retrieval Decision macro-F1 达到配置阈值；
- Relevance/Support 各类别无灾难性塌缩；
- Citation Precision、Citation Recall、Faithfulness 不低于传统 RAG 基线；
- 无需检索集上的过度检索率低于阈值；
- 不可回答集上的安全拒答率达到阈值；
- P95 延迟、平均检索轮次和每请求 token 成本在预算内；
- Tokenizer/model hash 匹配，MCP E2E 与回退测试通过；
- 人工抽检未发现隐藏 Prompt、密钥、训练数据或 Reflection Token 泄漏。

---

### 3.9 Student Serving、模型注册与回退

新增 `ModelRegistry` 管理 `model_id -> artifact_path/tokenizer_hash/dataset_version/metrics/status`，默认使用本地 JSON/SQLite，未来可替换 MLflow Registry。在线 `FineTunedReflectionModel` 支持：

- Transformers 本地推理；
- vLLM/OpenAI-Compatible endpoint；
- Ollama/LM Studio 等已有本地 Provider（能力满足 logprobs/特殊 Token 时）；
- Adapter 动态加载或合并后模型；
- 量化模型，但必须单独评估 Reflection Token 校准是否退化。

模型状态：`candidate -> validated -> deployable -> active -> archived`。切换 active 模型必须是原子操作；加载失败时保留旧模型并记录事件。

在线降级顺序：

1. Fine-tuned Self-RAG 正常路径；
2. Student 解析失败时最多一次格式修复/重新生成；
3. `hybrid` 模式交给 Prompted Teacher 复核；
4. Teacher 不可用或超预算时回退传统 Hybrid RAG；
5. 检索与生成均不可用时返回结构化错误，不生成无证据答案。
...
原架构层级保持不变，只在 Core Query 链路旁新增 Self-RAG 控制面，并增加离线 Training 子系统：
...
```
                              MCP Client / CLI / Dashboard
                                         ...
                                         ▼
┌──────────────────────────────────────────────────────────────────────┐
│ MCP Server / Tool Layer（保持 query_knowledge_hub 向后兼容）        │
└──────────────────────────────────┬───────────────────────────────────┘
                                   │
                                   ▼
┌──────────────────────────────────────────────────────────────────────┐
│ Core Query Layer                                                     │
│ QueryProcessor ─► SelfRAGOrchestrator ─► ResponseBuilder             │
│                       │                  │                            │
│                       │                  └► Citation/Multimodal       │
│                       ├► RetrievalGate                               │
│                       ├► Evidence/Support/Utility Critics            │
│                       ├► SegmentGenerator / QueryRewriter             │
│                       └► SelfRAGPolicy（预算、阈值、终止与回退）      │
└───────────────────────┬──────────────────────┬───────────────────────┘
                        │                      │
                        ▼                      ▼
┌───────────────────────────────┐  ┌───────────────────────────────────┐
│ 原 Retrieval Layer            │  │ Reflection Model Layer            │
│ Dense + Sparse + RRF + Rerank │  │ Prompted / FineTuned / Hybrid     │
└───────────────────────────────┘  └───────────────────────────────────┘
                        │                      │
                        └──────────┬───────────┘
                                   ▼
...
│ Source Snapshot → Synthetic Query → Retrieval Candidates             │
│ → Kimi/DeepSeek Teacher Labeling → QC/Split/Manifest                 │
│ → Critic SFT → Offline Reflection Labeling → Generator SFT/DPO       │
│ → Evaluation → Model Registry → Validated Artifact                   │
└──────────────────────────────────────────────────────────────────────┘
```

关键依赖方向：

- 在线 `core/self_rag` 可以依赖 `core/retrieval`、`libs/llm` 与 `libs/vector_store`；
- `core/retrieval` 不反向依赖 Self-RAG，确保传统 RAG 可独立运行；
- `src/training` 可以复用 Ingestion/Query 的领域类型与工厂，但在线服务不得 import Trainer、CUDA、DeepSpeed 等重依赖；
- Teacher Provider 与 Student Provider 使用不同配置命名空间和凭据；
- Dashboard/Observability 读取 Trace 与 Registry，不直接驱动训练内部状态。

#### 5.7.2 新增目录（相对原 5.2 节的增量）
...
│   ├── prompts/
│   │   └── self_rag/
│   │       ├── runtime/                 # retrieval/relevance/support/utility/rewrite
│   │       └── teacher/                 # 版本化 Teacher 标注 Prompt
│   ├── self_rag.yaml                    # 在线策略与控制 Token 配置
│   ├── dataset.yaml                     # 样本构建、过滤、切分、预算
│   └── training.yaml                    # 训练后端、LoRA、checkpoint、tracking
├── src/
│   ├── core/
│   │   ├── self_rag_types.py
│   │   └── self_rag/
...
│   ├── libs/
│   │   └── reflection/
│   │       ├── base_reflection_model.py
│   │       ├── prompted_reflection_model.py
│   │       ├── finetuned_reflection_model.py
│   │       ├── hybrid_reflection_model.py
...
│   │   ├── preference_pipeline.py
│   │   ├── evaluator.py
│   │   ├── model_registry.py
│   │   └── run_manifest.py
│   └── observability/dashboard/pages/
│       ├── self_rag_traces.py            # 检索决策/反思/重试可视化
│       └── training_monitor.py            # 数据版本/Run/指标/模型状态
├── scripts/
│   ├── build_selfrag_dataset.py
│   ├── label_selfrag_data.py
│   ├── train_selfrag_critic.py
│   ├── build_generator_data.py
│   ├── train_selfrag_generator.py
│   ├── evaluate_selfrag.py
│   └── promote_selfrag_model.py
├── data/training/                         # 数据内容默认 gitignore
├── artifacts/
│   ├── runs/                              # 指标与 manifests
│   ├── models/                            # checkpoint/adapter/tokenizer
│   └── registry/                          # 模型注册信息
└── tests/
    ├── fixtures/self_rag/
    ├── unit/self_rag/
    ├── unit/training/
    ├── integration/test_self_rag_pipeline.py
    ├── integration/test_teacher_labeling.py
    ├── integration/test_training_smoke.py
    └── e2e/test_self_rag_mcp.py
```

`data/training/raw`、Teacher cache、checkpoint 和模型权重默认不进入 Git；仓库只保留小型脱敏 Fixture、Schema、Manifest 示例和复现配置。

#### 5.7.3 Self-RAG Query Trace

在原 `TraceContext` 上增加可选事件，不破坏 ingestion/query 现有字段：

```json
{
  "trace_type": "self_rag_query",
  "query": "...",
  "mode": "finetuned",
  "model_id": "selfrag-student-v0.1.0",
  "policy_version": "policy-v1",
  "stages": [
    {"name": "retrieve_decision", "label": "retrieve", "confidence": 0.91},
    {"name": "retrieve", "round": 1, "candidate_count": 20},
    {"name": "evidence_critique", "kept": 4, "dropped": 16},
    {"name": "segment_generate", "segment": 0, "evidence_ids": ["chunk_001"]},
    {"name": "support_critique", "label": "fully_supported", "confidence": 0.88},
    {"name": "utility_critique", "label": 5},
    {"name": "terminate", "reason": "accepted"}
  ],
  "fallback": null,
  "latency_ms": 842,
...
Trace 不保存隐藏 reasoning、完整 Teacher 原始响应或训练文本正文。Dashboard 用标签、概率、证据 ID、耗时、轮次和 fallback reason 解释系统行为。

#### 5.7.4 配置增量

在原 `config/settings.yaml` 中只增加在线选择器；详细数据/训练配置拆分到独立文件：

```yaml
self_rag:
  enabled: true
  mode: finetuned             # disabled | prompted | finetuned | hybrid
  model_id: selfrag-student-v0.1.0
  tokenizer_manifest: ./artifacts/models/selfrag-student-v0.1.0/tokenizer_manifest.json
  fallback_mode: conventional_rag
  policy:
    ...
    max_generation_segments: 8
    max_total_context_tokens: 24000
    wall_time_ms: 8000
  decoding:
    beam_size: 1
    weights:
...
阈值和权重由 dev set 校准；配置默认值只用于首轮实验，不能未经评估直接宣称为最佳值。

#### 5.7.5 Dashboard 调整

保留原六页面，增量增加两个页面：

1. **Self-RAG Trace**：展示 retrieve/no-retrieve、各轮 Query、证据过滤、分段支持度、效用、fallback 与耗时；
2. **Training Monitor**：展示 Dataset 版本、Teacher 成本、标签分布、训练 Run、核心指标与 Model Registry 状态。

训练启动仍以 CLI 为主。Dashboard 默认只读，不直接执行高成本 Teacher 标注、GPU 训练或模型发布；如未来加入按钮，必须有二次确认、预算预览和权限控制。
