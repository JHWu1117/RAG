...
def _optional_setting(config: Any, name: str, expected_type: type) -> Any:
    """Return a concrete optional setting, ignoring dynamic Mock attributes."""
...
class OpenAIEmbedding(BaseEmbedding):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.dimensions = _optional_setting(settings.embedding, "dimensions", int)
        
        # API key: explicit > settings > env var
        self.api_key = (
            api_key
            or _optional_setting(settings.embedding, "api_key", str)
        ...
        azure_endpoint = _optional_setting(settings.embedding, "azure_endpoint", str)
        self.api_version = _optional_setting(settings.embedding, "api_version", str)
        ...
        if base_url:
            ...
            deployment = (
                _optional_setting(settings.embedding, "deployment_name", str)
                or self.model
            )
            ...
            settings_base_url = _optional_setting(settings.embedding, "base_url", str)
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        # Add dimensions for compatible models that support dimension control.
        # Legacy models such as text-embedding-ada-002 reject this parameter.
        dimensions = kwargs.get("dimensions", self.dimensions)
        dimension_models = ("text-embedding-3", "text-embedding-v4")
        if dimensions is not None and self.model.startswith(dimension_models):
