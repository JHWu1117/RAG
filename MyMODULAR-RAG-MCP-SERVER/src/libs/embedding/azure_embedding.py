    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.deployment_name = (
            _optional_setting(settings.embedding, "deployment_name", str) or
        ...
        # API key: explicit parameter > environment > settings.yaml.  This
        # prevents checked-in placeholder values from shadowing real secrets.
        self.api_key = (
            api_key or
            os.environ.get("AZURE_OPENAI_API_KEY") or
            os.environ.get("OPENAI_API_KEY") or
            _optional_setting(settings.embedding, "api_key", str)
        )
        if not self.api_key:
            raise ValueError(
        ...
        # Azure endpoint follows the same precedence as the API key.
        self.azure_endpoint = (
            azure_endpoint or
            os.environ.get("AZURE_OPENAI_ENDPOINT") or
            _optional_setting(settings.embedding, "azure_endpoint", str)
        )
        ...
        self.api_version = (
            api_version or
            _optional_setting(settings.embedding, "api_version", str) or
