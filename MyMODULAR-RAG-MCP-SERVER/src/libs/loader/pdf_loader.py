...
class PdfLoader(BaseLoader):
    ...
    def load(self, file_path: str | Path) -> Document:
        ...
        # Keep page boundaries as transient ingestion metadata.  MarkItDown
        # exposes a single document string, which is insufficient for page-level
        # citations after chunking.  PyMuPDF gives us deterministic per-page text
        # while the public Document.text remains the MarkItDown representation.
        page_texts = self._extract_page_texts(path)
        if page_texts:
        ...
        if page_texts:
            # Associate extracted image placeholders with their actual pages so
            # multimodal enrichment remains available in page-aware chunking.
            for image in metadata.get("images", []):
                page = image.get("page")
                image_id = image.get("id")
                if isinstance(page, int) and 1 <= page <= len(page_texts) and image_id:
                    page_texts[page - 1] += f"\n[IMAGE: {image_id}]\n"
    ...
    def _extract_page_texts(self, pdf_path: Path) -> List[str]:
        """Extract text per physical PDF page for citation-aware chunking.

        Failure is non-fatal: callers retain the original MarkItDown text and
        traditional document-level chunking behavior.
        """
        ...
        try:
            ...
            logger.warning(
                "Page-aware text extraction failed for %s, using document-level text: %s",
                pdf_path,
                exc,
            )
            return []
