...
class ChromaStore(BaseVectorStore):
    ...
    def close(self) -> None:
        """Close Chroma's persistent client so Windows releases SQLite files."""
        client = getattr(self, "client", None)
        # Collection objects retain the underlying Rust/SQLite API.  Drop that
        # reference before closing the shared client; otherwise Windows can
        # keep ``chroma.sqlite3`` locked after the test/application lifecycle.
        self.collection = None
        close = getattr(client, "close", None)
        if callable(close):
            close()
        self.client = None
        # Chroma's Rust bindings may form reference cycles.  A collection is
        # commonly closed immediately before a temporary directory is removed,
        # so collect them now instead of waiting for a later GC pass.
        import gc
        gc.collect()
