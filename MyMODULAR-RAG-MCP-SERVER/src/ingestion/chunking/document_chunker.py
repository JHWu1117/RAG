...
class DocumentChunker:
    ...
    def split_document(self, document: Document) -> List[Chunk]:
        ...
        # PDF loaders may provide transient per-page text. Split each page
        # independently so citations retain the physical source page.
            ...
            for page_num, page_text in enumerate(page_texts, start=1):
                if isinstance(page_text, str) and page_text.strip():
                    page_fragments.extend(
                        (fragment, page_num)
                        for fragment in self._splitter.split_text(page_text)
                    )
            text_fragments = [fragment for fragment, _ in page_fragments]
        else:
        ...
        for index, text in enumerate(text_fragments):
            ...
            page_num = page_fragments[index][1] if page_fragments else None
            chunk_metadata = self._inherit_metadata(
                document, index, text, page_num=page_num
            )
    ...
    def _inherit_metadata(
        ...
        chunk_metadata.pop("page_texts", None)
        ...
        if chunk_images and page_num is None:
