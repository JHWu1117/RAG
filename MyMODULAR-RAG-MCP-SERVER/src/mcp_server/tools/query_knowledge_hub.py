...
class QueryKnowledgeHubTool:
    ...
    def __init__(
        ...
        self._hybrid_search_injected = hybrid_search is not None
    ...
    def _ensure_initialized(self, collection: str) -> None:
        ...
        """
        # An explicitly supplied search implementation owns its dependencies
        # and lifecycle.  Honouring it keeps the public dependency-injection
        # contract usable for local/offline callers without constructing live
        # embedding or vector-store providers behind their back.
        if self._hybrid_search_injected:
            self._current_collection = collection
            self._initialized = True
            logger.info(
                "Using injected query components for collection: %s",
                collection,
            )
            return
