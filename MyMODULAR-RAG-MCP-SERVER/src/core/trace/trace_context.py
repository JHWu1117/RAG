...
class TraceContext:
    ...
    def elapsed_ms(self, stage_name: Optional[str] = None) -> float:
        ...
        # Some Windows clocks can return the same tick for two immediate
        # reads.  A live trace still represents a positive-duration event,
        # so retain a tiny non-zero value instead of reporting a false zero.
        return max((end - self._start_mono) * 1000.0, 1e-9)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_elapsed_ms": max(round(self.elapsed_ms(), 3), 0.001),
