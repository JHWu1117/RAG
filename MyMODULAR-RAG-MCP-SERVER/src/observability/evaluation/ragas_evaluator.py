...
def _import_ragas() -> None:
    """Validate that the optional Ragas distribution is installed.

    Do not import the package here: importing Ragas eagerly also imports all
    provider integrations, which can fail even when callers only need the
    evaluator's validation and mocked/offline paths.
    """
    try:
        installed = importlib.util.find_spec("ragas") is not None
