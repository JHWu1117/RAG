...
class TestVisionLLMFactory:
    ...
    def test_openai_vision_base_url_from_settings(self):
        """Vision provider must honor an OpenAI-compatible YAML endpoint."""
        class FakeSettings:
            class LLM:
                model = "qwen-plus"
                temperature = 0.0
        ...
        vision = OpenAIVisionLLM(FakeSettings(), api_key="test-key")

        assert vision.base_url == FakeSettings.vision_llm.base_url
        assert vision._use_azure_auth is False
