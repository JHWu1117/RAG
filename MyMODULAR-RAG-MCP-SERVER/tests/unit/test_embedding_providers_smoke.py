...
class TestOpenAIEmbedding:
    ...
    @patch('openai.OpenAI')
    def test_text_embedding_v4_sends_configured_dimensions(
        self, mock_openai_class: Mock, mock_settings_openai: Any
    ) -> None:
        """DashScope text-embedding-v4 supports explicit output dimensions."""
        mock_settings_openai.embedding.model = "text-embedding-v4"
        mock_settings_openai.embedding.dimensions = 1024
        mock_response = Mock()
        ...
        embedding = OpenAIEmbedding(mock_settings_openai, api_key="test-key")
        embedding.embed(["测试向量"])

        mock_client.embeddings.create.assert_called_once_with(
            input=["测试向量"], model="text-embedding-v4", dimensions=1024
        )
