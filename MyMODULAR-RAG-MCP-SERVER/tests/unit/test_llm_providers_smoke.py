...
class TestOpenAILLM:
    ...
    def test_base_url_from_settings(self):
        """OpenAI-compatible endpoints configured in YAML must be honored."""
        settings = MockSettings()
        settings.llm.base_url = "https://example.invalid/compatible-mode/v1"
        settings.llm.azure_endpoint = None
        settings.llm.api_version = None
