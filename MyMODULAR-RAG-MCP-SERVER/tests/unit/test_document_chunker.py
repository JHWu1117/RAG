...
def test_pdf_page_texts_produce_page_aware_chunks(chunker):
    """Transient PDF page text must become citation metadata, not vector metadata."""
    doc = Document(
        id="doc_pages",
        text="combined representation",
        metadata={
            "source_path": "paged.pdf",
            "doc_type": "pdf",
            "page_texts": ["First page.", "Second A.\n\nSecond B."],
        },
    )

    chunks = chunker.split_document(doc)

    assert [chunk.metadata["page"] for chunk in chunks] == [1, 2, 2]
    assert [chunk.metadata["page_num"] for chunk in chunks] == [1, 2, 2]
    assert all("page_texts" not in chunk.metadata for chunk in chunks)
