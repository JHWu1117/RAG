---
name: qa-tester
description: "Execute the Modular RAG MCP Server QA plan with terminal evidence across CLI, Streamlit AppTest, MCP JSON-RPC, provider switching, fault tolerance, and data lifecycle scenarios. Diagnose failures, apply bounded fixes, and record each result serially. Use for 'run QA', 'QA test', 'QA 测试', '执行测试', '跑测试', or test-and-fix requests; do not use for ordinary targeted unit-test runs."
---
...
> Pick ONE test → Run ONE command → Wait for output → Record ONE row in `.agents/skills/qa-tester/QA_TEST_PROGRESS.md` → THEN pick next.
...
> Use the virtual-environment interpreter directly when available: Windows `.\.venv\Scripts\python.exe`; macOS/Linux `.venv/bin/python`. If `.venv` is absent, stop and report the prerequisite instead of installing dependencies without permission.

---

## Step 1: Pick Target

1. Read `.agents/skills/qa-tester/QA_TEST_PLAN.md` for test steps and expected results.
2. Read `.agents/skills/qa-tester/QA_TEST_PROGRESS.md` for current status.
...
1. Read the test's **Steps** column from `.agents/skills/qa-tester/QA_TEST_PLAN.md`.
...
Edit `.agents/skills/qa-tester/QA_TEST_PROGRESS.md`: update ONE test row + summary counters. ONE row per file edit.
...
| File | Purpose |
|------|---------|
| `.agents/skills/qa-tester/QA_TEST_PLAN.md` | Test steps and expected results |
| `.agents/skills/qa-tester/QA_TEST_PROGRESS.md` | Execution status and notes |
