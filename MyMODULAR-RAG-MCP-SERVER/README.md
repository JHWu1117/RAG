...
> 当前仓库的 A–I 阶段是传统 RAG baseline。`DEV_SPEC_selfrag.md` 中的 J–N Self-RAG
> 阶段尚未实施，下面的命令也不会启用 Self-RAG 训练、控制 token 或 critic 模型。
...
`main.py` 只验证配置和入口，不会主动调用 LLM。不要把真实密钥提交到仓库；复制并编辑
`config/settings.yaml` 时应通过运行环境注入凭据（例如 `OPENAI_API_KEY`、
`AZURE_OPENAI_API_KEY` 和 `AZURE_OPENAI_ENDPOINT`），仓库中的值仅是占位符。

### 配置速查

| 配置段 | 关键字段 | 用途 |
|---|---|---|
| `llm` | `provider`, `model`, `deployment_name` | 答案生成、可选的 chunk/metadata LLM 处理 |
| `embedding` | `provider`, `model`, `dimensions` | Dense embedding；维度必须与已有集合一致 |
| `vision_llm` | `enabled`, `provider`, `model` | 图片描述；关闭或失败时保留图片引用并降级 |
| `vector_store` | `persist_directory`, `collection_name` | Chroma 持久化目录与默认集合 |
| `retrieval` | `dense_top_k`, `sparse_top_k`, `fusion_top_k`, `rrf_k` | Dense/BM25/RRF 参数 |
| `rerank` | `enabled`, `provider`, `top_k` | 可选精排；离线 baseline 可保持关闭 |
| `observability` | `trace_enabled`, `trace_file` | JSONL trace 和日志 |
| `ingestion` | `chunk_size`, `chunk_overlap`, `batch_size` | PDF ingestion 和批处理参数 |

Self-RAG 本地配置见 `config/settings.selfrag.local.yaml`：Generator 使用本地
Qwen3-4B + bitsandbytes NF4 4-bit，Reranker 使用本地
`BAAI/bge-reranker-v2-m3`。Critic 数据格式、模型回退和构造流程见
`docs/selfrag_critic_data_pipeline.md`。

摄取 PDF 前先配置可用的 embedding provider；包含 LLM refine、metadata enrichment 或图片描述的
路径还需要对应 LLM。先用 dry-run 验证文件选择不会产生索引：

```powershell
.\.venv\Scripts\python.exe scripts\ingest.py --path .\docs --collection knowledge_hub --dry-run
.\.venv\Scripts\python.exe scripts\ingest.py --path .\docs --collection knowledge_hub
```
...
VS Code / GitHub Copilot 的 `.vscode/mcp.json` 示例（把路径换成当前仓库绝对路径）：
...
```json
{
  "servers": {
    "modular-rag": {
      "type": "stdio",
      "command": "C:\\path\\to\\MyMODULAR-RAG-MCP-SERVER\\.venv\\Scripts\\python.exe",
      "args": ["-m", "src.mcp_server.server"],
      "cwd": "C:\\path\\to\\MyMODULAR-RAG-MCP-SERVER"
    }
  }
}
```

Claude Desktop 的 `mcpServers` 配置使用相同 command/args/cwd：
...
启动 Dashboard（系统总览、数据浏览、Ingestion 管理、摄取追踪、查询追踪、评估面板）：
...
标记为 `llm` 的测试默认显示为 skipped；必须配置真实外部服务后使用 `--run-llm` 显式运行，
本次 baseline migration 不运行这些测试。
常见故障排查：维度错误时删除测试集合后用一致 embedding 重建；MCP 无响应时先运行 `main.py`
检查配置并确认 client 的 `cwd`；Dashboard 无数据时检查 `vector_store.persist_directory` 和 collection；
Windows 上 Chroma 文件被占用时应先正常退出 MCP、Dashboard 和测试进程。
...
| **Cross-Encoder Reranker** | 已完成本地烟测 | Self-RAG 配置使用中文/多语种 `BAAI/bge-reranker-v2-m3`；模型权重位于 gitignored `artifacts/models/` |
